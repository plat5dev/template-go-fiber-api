...
func (s *Store) ListByProject(ctx context.Context, orgID, projectID string) ([]Task, error) {
	...
	for rows.Next() {
		var t Task
		if err := rows.Scan(&t.ID, &t.OrganizationID, &t.ProjectID, &t.Title, &t.Status, &t.CreatedByMembershipID, &t.CreatedAt, &t.UpdatedAt); err != nil {
			return nil, err
		}
		out = append(out, t)
	}
...
func (s *Store) FindInProject(ctx context.Context, orgID, projectID, taskID string) (*Task, error) {
	...
	var t Task
	err := row.Scan(&t.ID, &t.OrganizationID, &t.ProjectID, &t.Title, &t.Status, &t.CreatedByMembershipID, &t.CreatedAt, &t.UpdatedAt)
...
func (s *Store) Insert(ctx context.Context, t *Task) error {
	...
	_, err := s.db.ExecContext(ctx, `
		...
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
		t.ID, t.OrganizationID, t.ProjectID, t.Title, t.Status, t.CreatedByMembershipID, t.CreatedAt, t.UpdatedAt)
